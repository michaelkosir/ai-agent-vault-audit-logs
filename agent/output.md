...
3. **Path Wildcards**: For secret engine searches, use `LIKE 'path/%'`.
...
1. **Column Aliasing**: Use the `AS` keyword in your SQL to create user-friendly headers (e.g., `payload->'auth'->>'display_name' AS "User Name"`).
